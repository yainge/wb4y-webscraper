...
CONTRACT_NAME_BASES_CACHE: Optional[Dict[str, Any]] = None
CONTRACT_VARIANTS_CACHE: Optional[Dict[str, Any]] = None
CONTRACT_DURATIONS_CACHE: Optional[Dict[str, Any]] = None
...
def load_contract_reference_data() -> None:
    """
    Load contract reference data (bases, variants, durations) from JSON files.
    Caches data in global variables for performance.
    """
    global CONTRACT_NAME_BASES_CACHE, CONTRACT_VARIANTS_CACHE, CONTRACT_DURATIONS_CACHE
    ...
    try:
        ...
        if bases_file.exists():
            with open(bases_file, encoding="utf-8") as f:
                CONTRACT_NAME_BASES_CACHE = json.load(f)
            logger.info(f"Loaded contract_name_bases.json with {len([k for k in CONTRACT_NAME_BASES_CACHE if not k.startswith('_')])} providers")
        
        if variants_file.exists():
            with open(variants_file, encoding="utf-8") as f:
                CONTRACT_VARIANTS_CACHE = json.load(f)
            logger.info("Loaded contract_variants.json")
        
        if durations_file.exists():
            with open(durations_file, encoding="utf-8") as f:
                CONTRACT_DURATIONS_CACHE = json.load(f)
            logger.info("Loaded contract_durations.json")
    
    except Exception as e:
        logger.warning(f"Could not load contract reference data: {e}. Using fallback methods.")


def extract_contract_name_base_from_reference(
    contract_name: str,
    provider_name: str
) -> str:
    """
    Extract contract name base using reference data lookup.
    
    Strategy:
    1. Check contract_name_bases.json for provider's primary_bases
    2. Try to match contract_name to one of the primary_bases
    3. If no match, fall back to regex-based extraction
    
    Args:
        contract_name: Full contract name (after variant extraction)
        provider_name: Provider name for lookup
    
    Returns:
        Base contract name (best match or extracted)
    """
    if not contract_name or contract_name == 'unknown':
        return contract_name
    
    # If reference data not loaded, use fallback
    if not CONTRACT_NAME_BASES_CACHE:
        return extract_contract_name_base(contract_name)
    
    # Try to find provider in reference data
    ...
    if not provider_config or "primary_bases" not in provider_config:
        logger.debug(f"No reference bases for provider '{provider_name}', using fallback extraction")
        return extract_contract_name_base(contract_name)
    
    primary_bases = provider_config["primary_bases"]
    if not primary_bases:
        return extract_contract_name_base(contract_name)
    
    # Sort by length (longest first) to match most specific bases
    sorted_bases = sorted(primary_bases, key=len, reverse=True)
    
    # Try to match contract_name to a primary_base
    contract_lower = contract_name.lower()
    for base in sorted_bases:
        base_lower = base.lower()
        # Check if contract starts with or contains this base
        if contract_lower.startswith(base_lower):
            logger.debug(f"Matched contract '{contract_name}' to base '{base}' for provider '{provider_name}'")
            return base
        # Also match if base is the entire contract name
        if contract_lower == base_lower:
            return base
    
    # No reference match found, use fallback
    logger.debug(f"No reference match for contract '{contract_name}' in provider '{provider_name}', using fallback")
    return extract_contract_name_base(contract_name)
...
def transform_contract(
    ...
    # Use reference-based extraction if available, fallback to regex-based
    contract_name_simple = extract_contract_name_base_from_reference(contract_name_base, provider_name)
    ...
    # External feed-in lookup disabled for performance optimization
    # Feed-in data is sourced from in-file tariffs only
    # TODO: Re-implement with caching if external feed-in data becomes necessary
...
def main() -> int:
    ...
    # Load contract reference data (bases, variants, durations) at startup
    logger.info("Loading contract reference data...")
    load_contract_reference_data()
    logger.info("✓ Contract reference data loaded")
