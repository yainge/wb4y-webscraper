    ...
    async with async_playwright() as p:
        ...
        for y_pos in Y_POSITIONS:
            ...
            if status == 200 and tooltip_text and len(tooltip_text.strip()) > 0:
                try:
                    # Extract the inner tooltip HTML from VQL response structure
                    inner_html = None
                    try:
                        outer = json.loads(tooltip_text)
                        if isinstance(outer, dict) and "vqlCmdResponse" in outer:
                            cmd_list = outer["vqlCmdResponse"].get("cmdResultList", [])
                            if cmd_list and isinstance(cmd_list, list) and len(cmd_list) > 0:
                                tooltip_obj = cmd_list[0].get("commandReturn", {}).get("tooltipText", "")
                                if tooltip_obj:
                                    inner = json.loads(tooltip_obj)
                                    if isinstance(inner, dict) and "htmlTooltip" in inner:
                                        inner_html = inner["htmlTooltip"]
                    except Exception as e:
                        print(f"      ERROR extracting inner html: {e}")
                    
                    if not inner_html:
                        print(f"      SKIP: Could not extract inner HTML from response")
                        continue
                    
                    tooltip_dict = parse_tooltip_table(inner_html)
                    tuple_id = extract_tuple_id(tooltip_text)  # Still pass full response for tuple_id extraction
                    print(f"      DEBUG extracted tuple_id: '{tuple_id}' (len={len(tuple_id)})")
